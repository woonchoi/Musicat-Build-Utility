pipeline {
    ...
    environment {
        BUILD_BRANCH = "${env.gitlabBranch == 'dev-radio'}"
    }
    stages {
        ...
        stage("Set environment") {
            ...
            steps {
                echo "Copy require file to radioserver folder"
                sh 'cp /var/jenkins_home/util/radioserver/docker-compose.yml .'
                sh 'cp /var/jenkins_home/util/radioserver/Dockerfile ./radioserver'
                sh 'cp /var/jenkins_home/util/radioserver/.env ./radioserver'
            }
        }
        stage('Docker down') {
            when {
